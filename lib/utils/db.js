 ...
 * @returns {Promise}
 */
Db.prototype.mapReduce = function(Schema, map, reduce, finalize, scope) {
    var deferred = q.defer();
    var options = {
        map: map,
        reduce: reduce
    ...
    Schema.mapReduce(options, function(err, results) {
        deferred.resolve(results);
    });
 ...
 * @public
 */
Db.prototype.aggregate = function(Schema, pipeline) {
    var deferred = q.defer();

    Schema.aggregate(pipeline, function(err, results) {
        if (err) {
            deferred.reject(err);
        } else {
            deferred.resolve(results);
