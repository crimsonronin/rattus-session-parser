...
function setField(model, key, value) {
    ...
    if (!!fieldType && fieldType.instance) {
        logger.info('Setting the ' + fieldType.instance + ' value for the key: ' + key);
        ...
        logger.error('Could not find the field type for: ' + key);
    }
}
...
function mapTrial(trials, title, value) {
    ...
    if (!!title.value[1]) {
        ...
        if (!!value) {
            ...
            logger.error('The value for the key ' + title.key + ' is undefined');
        }
    }
}
...
function mapSession(data) {
    ...
    _.forEach(data, function(value, key) {
        ...
        if (key.indexOf('Trial') === 0) {
            ...
            if (!!title) {
                logger.info('Found a trial model key for: ' + key);
                mapTrial(session.trials, title, value);
            } else {
                logger.error('Could not find a trial model key for: ' + key);
            }
        } else {
            title = parseSessionTitle(key);
            if (!!title) {
                logger.info('Found a session model key for: ' + key);
                setField(session, title.key, value);
            } else {
                logger.error('Could not find a session model key for: ' + key);
            }
        }
    });
 ...
 * @returns {Promise}
 */
CsvDataMapperService.prototype.toModels = function(parsedData) {
    var models = [];
...
module.exports = new CsvDataMapperService();
